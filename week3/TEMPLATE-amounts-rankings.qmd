...
- visualizing amounts/rankings: showing the relationship between a numeric and categorical variable; comparing groups based on numerical values
- bar charts and lollipop charts are interchanegable
- heat maps (2 cat vars and 1 numeric)
- dumbbell charts (for visualizing change/difference between two groups)

- beware of truncating axes!! can really change the message that the plot sends
- when you want to encode by position (as opposed to length), truncating (axis not starting a zero) could be ok
- bar charts not actually the best for comparing counts! distribution-based plots could be better for continuous data
...
subregion_names <- tribble(             # used to construct data by hand
...
ca_region %>% 
  ...
  mutate(subregion_name = fct_reorder(.f = subregion_name, # which group you want to reorder
                                      .x = mean_sui_2010_2020)) %>%  # order by the mean column we just created
  ggplot(aes(x = mean_sui_2010_2020, y = subregion_name)) +
  geom_col() + 
  geom_text(aes(label = round(mean_sui_2010_2020, 2)),
            color = "white",
            hjust = 1.2)

#..........................lollipop plot.........................

# geom_alt package is an option for lollipop plots, but won't always work
...
ca_region %>% 
  ...
  mutate(subregion_name = fct_reorder(.f = subregion_name, # which group you want to reorder
                                      .x = mean_sui_2010_2020)) %>%  # order by the mean column we just created
  ggplot(aes(x = mean_sui_2010_2020, y = subregion_name)) +
  geom_point() + # places point where range ends/ value is
  geom_linerange(aes(xmin = 0, xmax = mean_sui_2010_2020)) + # could be ymin depending on chart orientation
  geom_text(aes(label = round(mean_sui_2010_2020, 2)), hjust = -0.3) + # negative values = nudge right
...
#.......................geom_bar() example.......................
# How many observations (rows) exist for each subregion?; geom_bar() calculates automatically!
ggplot(ca_region, aes(x = subregion_name)) +
  geom_bar() +
  coord_flip() +
  scale_y_continuous(label = scales::label_comma()) # adds commas to numbers with thousands!
```
...
sui_severity <- ca_region |>
  ...
  mutate(month = month.abb[month], # works because the original month column just has the month numbers
  ...
  mutate(sui_category = factor(sui_category,
                               levels = c("Very low", "Low", "Moderate", "High", "Severe"))) # spelling must match exactly!
```
...
ggplot(sui_severity, aes(x = month, y = tot_num_sui, fill = sui_category)) + 
  geom_col(position = "fill") + # converts axis to proportion
  scale_fill_manual(values = sui_colors) +
  scale_y_continuous(labels = scales::label_percent()) # changes prop to percent!
...
order_2015 <- heatmap_data %>% 
  ...
  ungroup() %>%                       # if don't ungroup, which just give each group a ranking of 1!
...
ggplot(heatmap_order, aes(x = year, y = subregion_name, fill = annual_mean_sui)) +
  geom_tile() +
  scale_fill_viridis_c() + # c for continuous data
  labs(fill = "Annual Mean Surface Water Supply and\nUse Index (SUI)") + #\n is where text will break
  guides(fill = guide_colorbar(title.position = "top", # puts legend title above the actual colorbar
                               barwidth = 15, barheight = 0.75)) +
   scale_x_continuous(breaks = seq(2010, 2020, by = 2)) +
  theme_minimal() +
  theme(legend.position = "top", # moves legend above the plot!
    panel.grid = element_blank(),
    axis.title = element_blank())  # x axis is clearly a year, so can remove!

# plot didn't work lol
...
ggplot(sbc_subbasin_monthly) +
  ...
  geom_point(aes(x = mean_consum, y = month),
             color = "#9C7344",
             size = 5,
             shape = 21, fill = "white", stroke = 2) + # 21 is a fillable circle; stroke is outline of circle
