...
ggplot(lyme_pop, aes(x = year, y = cases_per100k, group = state)) +
  geom_line() +
  gghighlight(state == "Connecticut")

#..................highlight based on criteria...................
ggplot(lyme_pop, aes(x = year, y = cases_per100k, group = state, color = state)) +
  geom_line() + 
  gghighlight(max(cases_per100k > 100)) # only highlight states where there are more than 100 cases per 100k people
```
...
Lecture notes:
- slopes of lines look different depending on aspect ratio that is used
...
lyme_pop |> 
  ...
  coord_fixed(ratio = 1/50) # changing aspect ratio ("1 change in x is equal to 50 changes in y")
```
...
lyme_pop |> 
  ...
  # remove blank space that ggplot automatically includes
  scale_x_discrete(expand = c(0, NA)) + # let ggplot decide what topmost value is
...
# top line: all of our groups collectively; areas are relative to other groups
# not good for showing values for each group
```

### Alternative: proportional stacked area chart

- plot percentage contributions rather than absolute numbers on y-axis
- makes it easier to interepret!
