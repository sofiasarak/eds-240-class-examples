...
## Lecture Notes
- use serif fonts sparingly
- lato, open-sans as a clean, sans-serif font
- look for options with lining gigures, or where numbers line up/are all the same height
- mono-spaced fonts preserve same width for every number (helpful for tables of numbers or axes)
- make sure symbols look right, as well
- use bold sparingly, to highlight ideas
- make sure font size is large enough 
- Cooler will rate contrast between text and background colors
- fontpair.com
- capitalization is personal preference

Annotate:
- tell your readers what you want them to see
- guide your reader's attention
- being repeitive is ok, communicate a lot in litle time!
...
#.........................create caption.........................
github_icon <- "&#xf09b"
github_username <- "sofiasarak"

caption <- glue::glue(
  "Data Source: USGS CONSUS 2024<br>
...
annotation <- glue::glue(
  "Santa Barbara meets deficits through diverse water source
...
water_plot <- ggplot(sbc_subbasin_monthly) +
  ...
  annotate(
    ...
    size = 3.5,
    hjust = "center",
    family = "noto_sans" # geom_text would plot text 12 times 
  ) +
  annotate(
    geom = "segment",
    x = 3.5, xend = 3.5, # ensures that it is a perfectly vertical line
    y = 4, yend = 1,
    linewidth = 0.8
  ) +
  annotate(
  ...
  labs(title = title,
       subtitle = "Summer deficits exist despite winter water surplus", # include plot takeaway as subtitle
       x = "Mean monthly water availability & consumption (mm)",
       caption = caption) +
  theme_light(base_size = 17) + 
  theme(
    plot.title.position = "plot", # left-aligns titles to plot instead of plot panel 
    ...
    plot.subtitle = element_text(family = "noto_sans",
                                 size = rel(0.9),
                                 color = pal["light_gray"],
                                 margin = margin(b = 8)), # set margin size of subtitle using first letter of bottom, top, left, right
    axis.text = element_text(family = "red",
                             size = rel(0.7),
                             color = pal["light_gray"]),
    axis.title.x = element_text(family = "noto_sans",
                                size = rel(0.75),
                                margin = margin(t = 15)), # more breathing room between plot and axis title
    plot.caption = ggtext:element_markdown(family = "noto_sans", 
                                face = italic,
                                size = rel(0.5),
                                color = pal["light_gray"],
                                margin = margin(t = 15),
                                lineheight = 1.5), # more breathing room between lines
    axis.title.y = element_blank().
    panel.grid.major.y = element_blank(),
    
    # add margin space around plot, especially helpful for saving .pngs
    plot.margin = margin(t = 1, r = 1, b = 1, l = 1, uniti = "cm")
    )
...
ggsave(
  ...
  plot = water_plot,
  device = "png", # graphics device
  width = 8,
  height = 7,
  unit = "in",
  dpi = 300 # default value
)
